...
const Hero = () => {
  const {axios,getToken,navigate,setRecentCities} = UseAppContext()
  ...
  const handleSearch = async(e)=>{
      ...
      setRecentCities((prevCities)=>{
        const updatedsearchcities = [...prevCities,destination]
        if(updatedsearchcities.length > 3){
          updatedsearchcities.shift()
        }
        console.log(updatedsearchcities)
  ...
  return (
    ...
    bg-no-repeat bg-cover h-screen'>
      ...
      <h1 className='font-playfair text-2xl md:text-5xl lg-text-[56px] md:leading-[56px]
      font-bold] md:font-extrabold max-w-xl mt-4 '>Discover Your Perfect Gateway Destination</h1>
      <p className='max-w-130 mt-2 text-sm md:text-base'>Unparalleled luxury and confort await at the world's most exclusive hotels and resorts.
