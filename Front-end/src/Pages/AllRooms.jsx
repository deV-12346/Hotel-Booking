       ...
       {filteredRooms.map((room)=>(
           ...
           <div className='md:w-1/2 flex flex-col  gap-2'>
            ...
            {room.amenities.map((item,index)=>(
                <div key={index} className='flex items-center justify-center gap-2 px-5 py-2 rounded-lg bg-[#f5f5ff]/70'>
                  <img src={facilityIcons[item]} alt={item}  className='w-5 h-5'/>
                  <p className='text-xs'>{item}</p>
                </div>
            ))}
            </div>
            <p className='text-xl font-medium text-gray-800'>{currency}{room.pricePerNight}/Night</p>
            </div> 
           </div>
       ))}
       </div>
       <div className='w-90 md:w-110 bg-white border md:ml-52 border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
          ...
          <div className={`${openfilters? "h-auto" : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
                ...
                {sortOption.map((option,index)=>(
                   ...
                   selected={selectSort === option} onChange={()=>handlesortChange(option)} />
                ))}
                </div>
          </div>
       </div>
